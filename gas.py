...
def RoeSolver2(uLin: torch.Tensor, uRin: torch.Tensor, gammaIn: torch.Tensor):
    ...
    L0Roe[L0Fix] = 0.5 * torch.sign(L0Roe[L0Fix]) * \
        (L0Roe[L0Fix]**2 + deltaEF[L0Fix]**2) / deltaEF[L0Fix]
    L4Roe[L4Fix] = 0.5 * torch.sign(L4Roe[L4Fix]) * \
        (L4Roe[L4Fix]**2 + deltaEF[L4Fix]**2) / deltaEF[L4Fix]
...
def F_u2F2(uIn: torch.Tensor, ux: torch.Tensor,
